...
class ContextoUsuario:
    def __init__(self, user_id: str, sp: spotipy.Spotify):
        self.user_id = user_id
        self.sp = sp

def obtener_contexto_usuario(request: Request) -> ContextoUsuario:
    """Valida la cookie de sesión y devuelve el contexto con el cliente de Spotify listo."""
    ...
    try:
        ...
        return ContextoUsuario(user_id=user_id, sp=sp)
    except Exception as e:
        print(f"❌ Error autenticando sp para usuario {user_id}: {e}")
...
@contextmanager
def db_session():
    """Context manager para asegurar que la conexión siempre se cierra y hace commit/rollback."""
...
@app.get("/login")
def login_spotify(): # 👈 Lo inyectamos aquí
    scope = "user-read-recently-played user-top-read"
    
    # 🚀 Usamos la de Ngrok con HTTPS que ya está registrada en Spotify
    url = f"https://accounts.spotify.com/authorize?response_type=code&client_id={SPOTIFY_CLIENT_ID}&scope={scope}&redirect_uri={SPOTIFY_REDIRECT_URI}"
...
@app.get("/callback")
def callback_spotify(code: str, request: Request): # 👈 Inyectamos la request aquí
    print("🛸 Código recibido de Spotify, iniciando intercambio de tokens con Ngrok...")
    
    # 🚀 Mandamos exactamente la misma URL de Ngrok que usamos en el login
    ...
    # ==========================================
    # 🧠 COLD START & GUARDADO INTELIGENTE
    # ==========================================
    # Abrimos la conexión UNA sola vez. Si algo hace crash aquí dentro,
    # el db_session() se encarga del rollback. Si todo es GG, hace el commit.
    with db_session() as conn:
        cursor = conn.cursor()
        
        # --- A. VERIFICACIÓN Y COLD START ---
        ...
        if not cursor.fetchone():
            ...
            # 3. Guardamos el O.V.R. base de sus artistas top
        ...
        # --- B. GUARDAR OPERADOR ---
        # Lo metemos en la misma transacción para que vaya todo en un solo paquete
        if spotify_user_id:
            cursor.execute('''
    ...
    # Al salir de esta indentación, el db_session hace automáticamente:
    # 1. conn.commit() (si no hubo errores)
    # 2. conn.close()
...
def obtener_usuario_actual(request: Request):
    ...
    # 4. Validamos el resultado y devolvemos el JSON limpio FUERA del bloque de la BD
    ...
    return {"user_id": user_id, "nombre": nombre_usuario}    
...
def registrar_feedback(datos: FeedbackData):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error interno al registrar feedback")
    ...
    try:
        with db_session() as conn:
            ...
            cursor.execute('''
                ...
                FROM canciones c JOIN fatiga_actual f ON c.track_id = f.track_id
                WHERE f.pico_historico > 0 AND f.puntos_fatiga < 10 AND f.ultima_actualizacion < date('now', '-30 days')
        ...
        print(f"Error en obtener_canciones_stats: {e}")
        return {"error": "Fallo interno"}
...
def obtener_dashboard(auth_data: dict = Depends(obtener_spotify_cliente)):
    ...
    try:
        ...
        print(f"❌ Error cargando el dashboard: {e}")
        raise HTTPException(status_code=500, detail="Error interno del servidor")

@app.get("/radar")
def obtener_radar_filtrado(
    rango: str = "MAX", 
    ctx: ContextoUsuario = Depends(obtener_contexto_usuario)
):
    # 🎯 Capturamos el usuario inyectado desde la cookie de forma segura
    user_id = ctx.user_id
    ...
    try:
        with db_session() as conn:
            ...
            if filtro:
                # 🛡️ V2: Añadimos 'WHERE f.user_id = ?' y en la subquery de reproducciones también!
                cursor.execute('''
                    SELECT 
                    ...
                    FROM fatiga_actual f
                    INNER JOIN (
                        SELECT DISTINCT track_id FROM reproducciones 
                        WHERE user_id = ? AND played_at >= date('now', ?)
                    ) r ON f.track_id = r.track_id
                    WHERE f.user_id = ?
                ''', (user_id, filtro, user_id)) # 👈 Pasamos los parámetros ordenados limpios
            else:
                # 🛡️ Filtro total (MAX): Solo las canciones del usuario logueado
                cursor.execute('''
                    SELECT 
        ...
        print(f"❌ Error en radar dinámico para {user_id}: {e}")
        # Mantenemos el fallback a 0 por si peta la DB, fiabilidad a tope
        datos = {"fresh": 0, "warning": 0, "burnout": 0}
...
def obtener_joya_del_dia(auth_data: dict = Depends(obtener_spotify_cliente)):
    sp = auth_data["sp"]
    print("Obteniendo joya del día...")
    
    joya_raw = None
    
    # PASO 1: Consulta rápida a la BD para pescar la joya
    try:
        with db_session() as conn:  # <-- ¡Con paréntesis!
            cursor = conn.cursor()
            cursor.execute('''
                ...
                AND r.ultima_vez <= date('now', '-1 days')
                ORDER BY r.total_repros DESC
                LIMIT 1
            ''')
            joya_raw = cursor.fetchone()
    except Exception as e:
        print(f"❌ Error en la consulta de base de datos: {e}")
        raise HTTPException(status_code=500, detail="Error interno al buscar la joya")

    # Si no hay ninguna canción que cumpla los requisitos, salimos ya
    if not joya_raw:
        return {"error": "no_hay_joya"}

    # Convertimos a diccionario para manipularlo fuera de la BD
    j_dict = dict(joya_raw)
    track_id = j_dict['track_id']

    # PASO 2: Si está indocumentada, llamamos a Spotify FUERA del with de la BD
    if not j_dict.get('nombre'):
        print(f"🔍 Joya indocumentada detectada ({track_id}). Preguntando a Spotify...")
        try:
            # Usamos .track() que va directo al grano con el ID
            t = sp.track(track_id) 
            nombre = t['name']
            artista = t['artists'][0]['name']
            img = t['album']['images'][0]['url'] if t['album']['images'] else "https://via.placeholder.com/640"
            
            # Actualizamos los datos en el diccionario que enviaremos al frontend
            j_dict['nombre'] = nombre
            j_dict['artista'] = artista
            j_dict['imagen_url'] = img
            
            # PASO 3: Abrimos otra conexión rápida SOLO para guardar los datos nuevos
            with db_session() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT OR REPLACE INTO canciones (track_id, nombre, artista, imagen_url)
                    VALUES (?, ?, ?, ?)
                ''', (track_id, nombre, artista, img))
                print(f"💾 Guardada nueva canción en DB: {nombre} - {artista}")
                
        except Exception as e:
            print(f"❌ Error al conectar con Spotify o actualizar la info: {e}")
            # No lanzamos 500 para que al menos el frontend reciba el track_id aunque no tenga metadatos
    
    # Aseguramos el mapeo que espera tu JS
...
def buscar_canciones(q: str = ""):
    ...
    try:
        with db_session() as conn:
            ...
            # 🔥 CORRECCIÓN AQUÍ: Para que el LIKE funcione en SQL con comodines, 
            # el string debe mantener los '%' al principio y al final.
            query_busqueda = f"%{q.lower()}%"
            ...
            # 🔥 CORRECCIÓN CLAVE: El fetchall() tiene que estar DENTRO del con la conexión abierta
    ...
    # Mapeamos el JSON tranquilamente FUERA del bloque with, con la BD ya cerrada
    resultados = [
        {
            "track_id": r[0], 
            "nombre": r[1], 
